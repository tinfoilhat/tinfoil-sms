	...
	public void onReceive(Context context, Intent intent) {
		...
		if (bundle != null) {
			...
			if (pdus != null) {
				...
				// Log.v("message", messages[0].getMessageBody() +
				// messages[1].getMessageBody());
				/*
				 * Might have to update this so that it accounts for multiple
				 * messages received
				 */
				if (messages.length > -1) {

					/*
					 * Shows a Toast with the phone number of the sender, and
					 * the message. String smsToast = "New SMS received from " +
					...
					if (MessageService.dba.inDatabase(address)) {
						...
						if (MessageService.dba.isTrustedContact((address))
							...
							try {
								 ...
								 */
								if (ConversationView.sharedPrefs
										.getBoolean(
												QuickPrefsActivity.SHOW_ENCRYPT_SETTING_KEY,
												false)) {
