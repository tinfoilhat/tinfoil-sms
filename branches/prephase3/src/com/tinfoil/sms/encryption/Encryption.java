...
package com.tinfoil.sms.encryption;
...
public abstract class Encryption
{
     ...
     */
    public static String rot13(final String plaintext)
    {
        String ciphertext = "";
     ...
     * @param plaintext The plaintext to be encrypted
     */
    public static String aes_encrypt(final String password, final String plaintext) throws Exception
    {
        final byte[] secret_key = generateKey(password.getBytes());
        //byte[] secretKey = secret_key.getBytes();
        final byte[] clear = plaintext.getBytes();

        final SecretKeySpec secretKeySpec = new SecretKeySpec(secret_key, CIPHER_ALGORITHM);
        final Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM);
        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpec);

        final byte[] encrypted = cipher.doFinal(clear);
        final String encryptedString = Base64.encodeToString(encrypted, Base64.DEFAULT);
     ...
     */
    public static String aes_decrypt(final String password, final String ciphertext) throws Exception
    {
        final byte[] secret_key = generateKey(password.getBytes());
        ...
        final byte[] encrypted = Base64.decode(ciphertext, Base64.DEFAULT);
     ...
     */
    public static byte[] generateKey(final byte[] password) throws Exception
    {
        final byte[] key = new byte[KEY_SIZE];
        final MessageDigest sha = MessageDigest.getInstance("SHA-1");
        final byte[] hash = sha.digest(password);
